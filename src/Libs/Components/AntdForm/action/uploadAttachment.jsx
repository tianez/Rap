...
const uploadAttachment = Component =>
    class extends React.Component {
        ...
        handleChange = ({ attachments = this.state.attachments }) => {
            let { onChange, field, multiple } = this.props;
            let value = [];
            ...
            this.setState({ value });
            if (multiple) {
                onChange && onChange(field.name || field.id, JSON.stringify(value));
            } else {
                onChange && onChange(field.name || field.id, value[0].path);
            }
        };
        size = size => {
            let tip =
