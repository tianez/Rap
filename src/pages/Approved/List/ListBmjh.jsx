...
export default class ApprovedList extends Component {
    ...
    handleSubmit = async data => {
        this.props.history.push("/approved/bmjh");
    };
    render() {
        let { counts, initialPage } = this.state;
        const tabs = [{ title: "抄送给我的", sub: "0" }, { title: "我的部门计划", sub: "1" }];
        return (
            <Layout>
                <NavBar mode="light" icon={<LeftIcon />} rightContent={<span onClick={this.handleSubmit}>写计划</span>}>
                    部门计划
                </NavBar>
                <Content>
