...
export default class ApprovedList extends Component {
    ...
    handleSubmit = async data => {
        this.props.history.push("/approved/zhoubao");
    };
    render() {
        let { counts, initialPage } = this.state;
        const tabs = [{ title: "抄送给我的", sub: "0" }, { title: "我的周报", sub: "1" }];
        return (
            <Layout>
                <NavBar mode="light" icon={<LeftIcon />} rightContent={<span onClick={this.handleSubmit}>写周报</span>}>
                    周报
                </NavBar>
                <Content>
