...
export default class RecordList extends Component {
    ...
    handleReq = async (
        ...
        let res = await Request('answer/list', { params })
        if (res.success) {
            let { list, count } = res.data.answerInfoList
            this.setState({
    ...
    handleClick = async (data) => {
        ...
        history.push('/onlineexam/record/' + data.id)
    }
    render() {
        ...
        return (
            <div className={styles.Lazy}>
                <LazyWarper onScroll={this.handleScroll} className={styles.LazyWarper} >
                            ...
                            data.map(item => {
                                return (
                                    <Item
                                        ...
                                        thumb={<div className={styles.thumb}>{item.score}分</div>}
                                        multipleLine
                    ...
                    <div ref={ele => this.load = ele}></div>
                    {!hasNext ?
                        data.length > 0 && <div className={styles.loadmore}>没有更多数据了</div> :
                        !loading && <div onClick={this.handleReq} className={styles.loadmore}>更多数据加载中...</div>
                    }
                </LazyWarper>
            </div>
        )
